<?php 

// Liste des id preparation 


/*
id_nom_recherche 
id_nom_club_1
id_nom_club_2
id_nom_club_1
*/
 

?>
<!DOCTYPE html>
<html lang="en">
<head>
  <title>Bootstrap Example</title>
  <meta charset="utf-8">
  <link rel="icon" type="image/x-icon" href="data:image/jpeg;base64,">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.5.0/Chart.min.js"></script>
</head>
<body>
<div class="jumbotron">
        <h1 id="recherche_element"></h1>
        <h1 id="nom_recherche"> </h1>
        <h2 id="nom_cut_recherche"></h2>
      </div>
        
      <div class="container container_blue">
        <div class="row">
       
          <div class="col" title="Niveau Regional 2 ">
            <div class="div1_2" id="id_r1"></div>
            <div class="div1" title="Niveau Regional 2 ">R</div>
          

       

          </div>
      
          <div class="col" title="Niveau national">
            <div class="div2_2"   id="id_n"></div>

            <div class="div2" >N</div>
          </div>
          <div class="col" title="Niveau International">
            <div class="div3_2"  id="id_i" ></div>

            <div class="div3" >IA</div>
          </div>
     
          <div class="col" title="Niveau Regional">
            <div class="div4_2"  id="id_r2" ></div>

            <div class="div4" >IR</div>
          </div>
          <div class="col" title="Niveau Départemental">
            <div class="div5_2"  id="id_d" ></div>


            <div class="div5"  >D</div>
          </div>
        </div>
      </div>




      <div class="container">
        <div class="row">


        <div  style="width:100%;margin:auto">

        <h3>
         Nombre total homme et femme
        </h3>
        <canvas id="hommes_femmes" style="width:100%;max-width:600px"></canvas>





        <div class="femmes_class" style="margin-bottom:15px">Femmes</div>

        <div class="hommes_class" style="margin-bottom:15px">Hommes</div>

        <h3>
         Nombre total epreuve
        </h3>




     
        <canvas id="epreuves_total1" style="width:100%;max-width:600px"></canvas>
        <canvas id="epreuves_total2" style="width:100%;max-width:600px"></canvas>

        <canvas id="epreuves_total3" style="width:100%;max-width:600px"></canvas>
        <canvas id="epreuves_total4" style="width:100%;max-width:600px"></canvas>

   

    <div class="International_font_class" style="margin-bottom:15px">International</div>
    <div class="National_font_class" style="margin-bottom:15px">National</div>
    <div class="Regional_font_class" style="margin-bottom:15px">Regional</div>
    <div class="Iner_Region_font_class" style="margin-bottom:15px">Iner Region</div>
    <div class="Departemental_font_class" style="margin-bottom:15px">Departemental</div>
 
        </div>
       
        
     
 
        </div>
      </div>



  


 
      <body>
      
    </body>
 <link rel="stylesheet" href="style3.css">



<div id="chargement">
<img src="https://media.tenor.com/5Bg1bLVpl8cAAAAC/loading-chargement.gif" alt="" srcset="" id="img_sec"  >


<style>

  #chargement{
    width:50%; 
    margin:auto ; 
  }
  #img_sec{
    width:100%; 
    margin:auto ; 
    background-color:red ; 
  }
</style>

 
 

 




</style>
<script src="Operation.js"></script>

<script src="get_epreuve_nom_complet_list.js"></script>
<script src="Operation_list.js"></script>




 
<script src="scrypt_boucle2.js"></script>
          <script>


 
            var recherche_ = "get_club_nom_complet_array_2" ; 
            var recherche_element ="A. La Riviere De Corps" ; 
            var recherche_element ="Clll Armentieres" ; 
            var recherche_element ="Lille Metropole Athletisme*" ; 
            var recherche_element ="Lagardere Paris Racing" ; 
            var recherche_element ="Ca Montreuil 93" ; 
            var recherche_element ="Neuilly-Plaisance Sports" ; 


            



            








            

            var total_s = 0 ; 




            

            document.getElementById("id_n").style="height:300px";     

            
           // console.log(tab_sprint) ; 

            var information_g  ; 
            var xmlhttp = new XMLHttpRequest();
            xmlhttp.onreadystatechange = function() {
              if (this.readyState == 4 && this.status == 200) {
                var myObj = JSON.parse(this.responseText);
                document.getElementById("chargement").innerHTML="";    
 

                switch(recherche_) {
  case "get_club_nom_complet_array_2":
    document.getElementById("recherche_element").innerHTML="club recherche";     
    

 
    break;
 
 
}

 




 

//const myOperation = new Operation(myObj,tab_sprint,cotation_1);
//const myOperation = new Operation(myObj,cotation_2);
 


/*
for(var x = 0 ; x< 3 ; x ++){
  
  var  myOperation = new Operation(myObj,cotation_x[x],tab_sprint[x]);
  myOperation.boucle_() ;
  myOperation.myChart_(nom_canvas[x]) ; 

}
*/

 
var nombre = 2  ; 
  
var  myOperation = new Operation(myObj,cotation_x[nombre],tab_sprint[nombre]);
  myOperation.boucle_() ;
  console.log(myOperation.x) ;

  

  
  //myOperation.myChart_(nom_canvas[nombre]) ; 





//console.log(myOperation.array_1_info()) ; 
document.getElementById("nom_recherche").innerHTML=recherche_element;    
                
                 
                
              information_g = myObj ; 
 
 

 

 

// 




 



 



 

 
              
              

 









  




              



 

 





 
              
              
 
              }
            };
            xmlhttp.open("GET", "https://bokonzi.com/ffa/vlog.php/["+recherche_+"]/"+recherche_element, true);
            xmlhttp.send();







            const liste_elements_array = [
 
	'get_rp_array_2',	
	'get_vent_array_2',	
	'get_result_users_perf_array_2',	
	'get_result_users_nom_1_array_2',	
	'get_result_users_nom_2_array_2',	
	'get_result_users_nom_3_array_2',	
	'get_result_users_nom_4_array_2',	
	'get_users_nationality_array_2',	
	'get_club_nom_complet_array_2',	
	'get_club_departement_array_2',	
	'get_club_region_array_2',	
	'get_cat_array_2',	
	'get_users_naissance_array_2',	
	'get_result_date_perf_array_2',	
	'get_result_villes_nom_array_2',	
	'epreuve_sex_array_2',	
	'get_users_nom_complet_array',	
	'info_all_array_click',	
	'info_all_array_ip',	
	'info_all_array_src_name',	
	'window_location_href',	
	'get_epreuve_nom_complet',	
	'reg_date'
]

 
 
 

 




 



          </script>

      

<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.9.4/Chart.js"></script>

<script>

</script>