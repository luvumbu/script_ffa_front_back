...
function setAthletesFilter($niveaux, $annee, $nbH = 50, $nbF = 50, $layout = 'magazine', $clubFilter = '', $epreuveFilter = '', $filterCibleEnabled = false) {
    ...
    $clubFilter = trim((string)$clubFilter);
    $epreuveFilter = trim((string)$epreuveFilter);
    if (mb_strlen($clubFilter) > 100) $clubFilter = mb_substr($clubFilter, 0, 100);
    if (mb_strlen($epreuveFilter) > 100) $epreuveFilter = mb_substr($epreuveFilter, 0, 100);
    $f = __DIR__ . '/../logs/.athletes_filter.php';
...
if (isset($_GET['action']) && $_GET['action'] === 'preview_athletes') {
    ...
    $clubJoin = '';
    if ($afClubFilter !== '') {
        $clubEsc = $conn->real_escape_string($afClubFilter);
        $clubJoin = "INNER JOIN athlete_clubs ac_f ON ac_f.id_athlete = a.id_athlete
                     INNER JOIN clubs cl_f ON cl_f.id_club = ac_f.id_club AND cl_f.nom_club LIKE '%$clubEsc%'";
    }
    ...
    <form id="athletesFilter" method="POST" action="panel.php" style="background:#161b22;border:1px solid #30363d;border-radius:12px;padding:20px 22px;margin-bottom:20px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:18px;">
            <div>
                ...
                <select name="annee" id="afAnnee" style="width:100%;background:#0d1117;border:1px solid #30363d;border-radius:8px;padding:10px 14px;color:#f0f6fc;font-size:14px;font-weight:600;cursor:pointer;">
        ...
        <div style="background:linear-gradient(135deg,#0c4a6e15,#082f4908);border:1.5px solid #0891b2<?= $advFiltersOn ? '' : '40' ?>;border-radius:12px;padding:18px 20px;margin-bottom:22px;<?= $advFiltersOn ? 'box-shadow:0 4px 20px rgba(8,145,178,0.2);' : '' ?>">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
                <!-- Club -->
                <div style="position:relative;">
                    <label for="afClubFilter" style="display:block;color:#22d3ee;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">&#127963; Meme club</label>
                    <input type="text" name="club_filter" id="afClubFilter" autocomplete="off" value="<?= htmlspecialchars($afClubFilter) ?>" placeholder="Ex: ES MASSY ATHLETISME" style="width:100%;background:#0d1117;border:1px solid #0891b240;border-radius:8px;padding:10px 14px 10px 36px;color:#f0f6fc;font-size:13px;font-weight:600;box-sizing:border-box;">
                    <span style="position:absolute;left:12px;top:38px;color:#0891b2;font-size:14px;">&#128269;</span>
                    ...
                    <p style="color:#5a6580;font-size:10px;margin-top:5px;font-style:italic;">Tape 2+ caracteres pour autocompleter (recherche live).</p>
                </div>

                <!-- Epreuve -->
                <div style="position:relative;">
                    <label for="afEpreuveFilter" style="display:block;color:#22d3ee;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">&#127939; Meme epreuve</label>
                    <input type="text" name="epreuve_filter" id="afEpreuveFilter" autocomplete="off" value="<?= htmlspecialchars($afEpreuveFilter) ?>" placeholder="Ex: 100m, Longueur, Triple saut..." style="width:100%;background:#0d1117;border:1px solid #0891b240;border-radius:8px;padding:10px 14px 10px 36px;color:#f0f6fc;font-size:13px;font-weight:600;box-sizing:border-box;">
                    <span style="position:absolute;left:12px;top:38px;color:#0891b2;font-size:14px;">&#128269;</span>
                    ...
                    <p style="color:#5a6580;font-size:10px;margin-top:5px;font-style:italic;">Nom exact de l'epreuve (ou recherche live).</p>
                </div>
            </div>

            <?php if ($advFiltersOn): ?>
            <button type="button" onclick="document.getElementById('afClubFilter').value='';document.getElementById('afEpreuveFilter').value='';" style="margin-top:12px;background:transparent;color:#f87171;border:1px solid #f8717140;border-radius:6px;padding:6px 14px;font-size:11px;font-weight:600;cursor:pointer;">&times; Effacer les filtres cibles</button>
            <?php endif; ?>
        </div>
        ...
        (function(){
            // Autocompletion club
            var clubInput = document.getElementById('afClubFilter');
            var clubResults = document.getElementById('afClubResults');
            var clubTimer = null;
            if (clubInput) {
                clubInput.addEventListener('input', function(){
                    ...
                    clubTimer = setTimeout(function(){
                        fetch('panel.php?action=search_clubs&q=' + encodeURIComponent(q))
                          .then(function(r){ return r.json(); })
                          .then(function(d){
                            if (!d.results || !d.results.length) { clubResults.style.display = 'none'; return; }
                            clubResults.innerHTML = d.results.map(function(c){
                                return '<div class="afc-res" data-name="' + c.nom.replace(/"/g,'&quot;') + '" style="padding:8px 14px;cursor:pointer;border-bottom:1px solid #21262d;color:#f0f6fc;font-size:13px;"><strong>' + c.nom + '</strong> <span style="color:#8b949e;font-size:11px;">(' + c.nb + ' athletes)</span></div>';
                            }).join('');
                            clubResults.style.display = 'block';
                          });
                    }, 250);
                });
                clubResults.addEventListener('click', function(e){
                    var row = e.target.closest('.afc-res');
                    if (!row) return;
                    clubInput.value = row.dataset.name;
                    clubResults.style.display = 'none';
                });
                document.addEventListener('click', function(e){
                    if (!clubInput.contains(e.target) && !clubResults.contains(e.target)) clubResults.style.display = 'none';
                });
            }

            // Autocompletion epreuve
            var epInput = document.getElementById('afEpreuveFilter');
            var epResults = document.getElementById('afEpreuveResults');
            var epTimer = null;
            if (epInput) {
                epInput.addEventListener('input', function(){
                    clearTimeout(epTimer);
                    var q = epInput.value.trim();
                    if (q.length < 2) { epResults.style.display = 'none'; return; }
                    epTimer = setTimeout(function(){
                        fetch('panel.php?action=search_epreuves&q=' + encodeURIComponent(q))
                          .then(function(r){ return r.json(); })
                          .then(function(d){
                            if (!d.results || !d.results.length) { epResults.style.display = 'none'; return; }
                            epResults.innerHTML = d.results.map(function(c){
                                return '<div class="afe-res" data-name="' + c.nom.replace(/"/g,'&quot;') + '" style="padding:8px 14px;cursor:pointer;border-bottom:1px solid #21262d;color:#f0f6fc;font-size:13px;"><strong>' + c.nom + '</strong> <span style="color:#8b949e;font-size:11px;">(' + c.nb + ' records)</span></div>';
                            }).join('');
                            epResults.style.display = 'block';
                          });
                    }, 250);
                });
                epResults.addEventListener('click', function(e){
                    var row = e.target.closest('.afe-res');
                    if (!row) return;
                    epInput.value = row.dataset.name;
                    epResults.style.display = 'none';
                });
                document.addEventListener('click', function(e){
                    if (!epInput.contains(e.target) && !epResults.contains(e.target)) epResults.style.display = 'none';
                });
            }
        })();
        </script>
        ...
        <div style="background:#0d1117;border:1px solid #30363d;border-radius:10px;padding:16px;margin-bottom:18px;">
            ...
            <div id="afSchema" style="display:flex;flex-direction:column;gap:10px;">
                <div style="background:linear-gradient(135deg,#a78bfa15,#a78bfa05);border:1px dashed #a78bfa60;border-radius:6px;padding:10px;text-align:center;font-size:11px;color:#a78bfa;">
                    📰 ÉLITE <span id="schemaYr"><?= $afAnnee ?></span> &middot; <span id="schemaNiv"><?= htmlspecialchars(implode(' & ', $afNiveaux)) ?></span>
                </div>
        ...
        <?php if ($afSaved):
            $afEpreuvesPub = '100m|200m|400m Haies (76)|400m Haies (91)|110m Haies (91)|110m Haies (99)|110m Haies (106)|Longueur|Triple saut|Perche';
...
(function() {
    ...
    function updateYr() {
        if (spanYr && afYear) spanYr.textContent = afYear.value;
    }
...
(function() {
    ...
    function buildUrl(sx, lim, niveau) {
        return '/api/liste.php?limit=' + lim + '&ordre=medailles&niveau=' + encodeURIComponent(niveau)
             + '&recent_year_min=' + annee
